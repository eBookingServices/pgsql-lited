...
struct Inserter(ConnectionType) {
	...
	void row (T) (ref const T param) if (!isValueType!T) {
		...
		if (start_.empty)
			throw new PgSQLErrorException("Inserter must be initialized with a call to start()");
		...
		foreach (i, ref fieldHash; fieldsHash_) {
			...
			if (!fieldFound)
				throw new PgSQLErrorException(format("field '%s' was not found in struct => '%s' members", fieldsNames_.ptr[i], typeid(Unqual!T).name));
	...
	void row(Values...)(Values values) if(allSatisfy!(isValueType, Values)) {
		...
		if (valueCount != fields_)
			throw new PgSQLErrorException(format("Wrong number of parameters for row. Got %d but expected %d.", valueCount, fields_));
